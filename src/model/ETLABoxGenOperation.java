...
public class ETLABoxGenOperation implements ETLOperation{
	...
	public boolean getInput(JPanel panel, HashMap<String, LinkedHashSet<String>> inputParamsMap) {
		...
		btnNewMGFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String filePath = commonMethods.getFilePath("Select R2RML Mapping File");
				if(!filePath.equals("")){
		...
		btnNewOutputDir.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				defaultName+="_"+commonMethods.getDateTime();
		...
		if (confirmation == JOptionPane.OK_OPTION) {
			...
			boolean hasEmpty = commonMethods.hasEmptyString(stringList);
			if(!hasEmpty){
				...
				if (aBoxFilePath.equals("None")) {
					commonMethods.showMessage("Please select path for saving ABox file.");
					return false;
	...
	public boolean execute(JTextPane statusPane) {
		...
		if (dbConnection == null) {
			...
			boolean flag = commonMethods.writeFile(outFilePath, rdfString);
			if(flag){
