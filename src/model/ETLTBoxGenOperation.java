...
public class ETLTBoxGenOperation implements ETLOperation {
		...
		btnNew.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				String filePath = commonMethods.getSaveFile("", defaultName,
						"Select Directory to save TBox file");
		...
		if (confirmation == JOptionPane.OK_OPTION) {
			...
			if (!hasEmpty) {
				...
				if (filePath.equals("None")) {
					commonMethods.showMessage("Please select path for saving TBox file.");
					return false;
	...
	public boolean execute(JTextPane statusPane) {
		...
		if (dbConnection == null) {
			...
			boolean flag = commonMethods.writeFile(tBoxFileSavingPath, tBoxString);
			if (flag) {
